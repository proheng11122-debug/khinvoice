...
export function SettingsPage() {
  ...
  async function handleQRUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const { error: uploadError } = await supabase.storage.from('qr-codes').upload(fileName, file, { upsert: true })
    if (uploadError) { setError(uploadError.message); setUploading(false); return }

    const { data } = supabase.storage.from('qr-codes').getPublicUrl(fileName)

    const { error: updateError } = await supabase.from('profiles').update({ qr_code_url: data.publicUrl }).eq('id', session!.user.id)
    if (updateError) { setError(updateError.message) } else { setMessage('QR code uploaded successfully'); await refreshProfile() }
    setUploading(false)
  }
      ...
      <div className="card">
        ...
        {profile?.qr_code_url && (
          <div className="text-center mb-4">
            <img src={profile.qr_code_url} alt="Payment QR Code" style={{ maxWidth: 200, borderRadius: 12, border: '1px solid var(--border)' }} />
          </div>
        )}

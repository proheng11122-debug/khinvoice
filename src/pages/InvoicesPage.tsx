  ...
  return (
    <div>
      ...
      <div className="stat-grid">
        ...
        <div className="stat-card"><div className="stat-label">Total Paid</div><div className="stat-value income">{formatCurrency(totalPaid)}</div></div>
        <div className="stat-card"><div className="stat-label">Outstanding</div><div className="stat-value expense">{formatCurrency(totalOutstanding)}</div></div>
      </div>
      ...
      <div className="card">
        ...
        {filtered.length === 0 ? (
          <div className="empty-state"><div className="empty-state-icon">I</div><p>No invoices yet. Click "New Invoice" to create one.</p></div>
        ) : (
          <div className="table-wrapper">
            <table>
              <thead><tr><th>#</th><th>Customer</th><th>Date</th><th>Due Date</th><th>Subtotal</th><th>Paid</th><th>Balance</th><th>Status</th></tr></thead>
              <tbody>
                {filtered.map((inv) => (
                  <tr key={inv.id}>
                    <td><Link to={`/invoices/${inv.id}`} style={{ fontWeight: 700 }}>#{inv.invoice_number}</Link></td>
                    ...
                    <td><span className="badge" style={{ background: `${statusColor(inv.status)}15`, color: statusColor(inv.status) }}>{inv.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>

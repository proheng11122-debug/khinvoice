  ...
  useEffect(() => {
    async function loadData() {
      ...
      const [txRes, invRes, prodRes] = await Promise.all([
        ...
        supabase.from('invoices').select('*').eq('user_id', userId).order('created_at', { ascending: false }).limit(5),
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        <div className="card">
          ...
          {transactions.length === 0 ? (
            <div className="empty-state"><div className="empty-state-icon">T</div><p>No transactions yet</p></div>
          ) : (
            <div className="table-wrapper">
              <table>
        ...
        <div className="card">
          ...
          {invoices.length === 0 ? (
            <div className="empty-state"><div className="empty-state-icon">I</div><p>No invoices yet</p></div>
          ) : (
            <div className="table-wrapper">
              <table>
                <thead><tr><th>#</th><th>Customer</th><th>Status</th><th>Amount</th></tr></thead>
                <tbody>
                  {invoices.map((inv) => (
                    <tr key={inv.id}>
                      <td><Link to={`/invoices/${inv.id}`} style={{ fontWeight: 600 }}>#{inv.invoice_number}</Link></td>
                      <td>{inv.customer_name}</td>
                      <td>
                        <span className="badge" style={{ background: `${statusColor(inv.status)}15`, color: statusColor(inv.status) }}>
                          {inv.status}
                        </span>
                      </td>

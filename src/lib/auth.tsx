...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, newSession) => {
      ...
      if (newSession?.user) { (async () => { await fetchProfile(newSession.user.id); setLoading(false) })() }
      else setLoading(false)
    })
  ...
  async function signUp(phone: string, password: string, businessName: string, username: string) {
    ...
    if (data.user) {
      const { error: pe } = await supabase.from('profiles').insert({ id: data.user.id, business_name: businessName, username, phone })
  ...
  async function refreshProfile() { if (session?.user) await fetchProfile(session.user.id) }
